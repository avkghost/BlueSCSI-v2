...
BlueSCSI is hardware that reproduces SCSI devices (hard disks, optical, etc) powered by the Raspberry Pi RP2040/RP2350.

This repository contains code for both BlueSCSI v2 and BlueSCSI Ultra/Ultra Wide.

image::docs/buynow.png[Buy Now,link=https://scsi.blue,width=150]
image::docs/discordbanner.png[Join our Discord,link=https://discord.gg/GKcvtgU7P9,width=150]
...
### Network Bridge Terms

1. `DaynaPORT in raw bridge mode` - the USB initiator path that forwards SCSI traffic without interpreting DaynaPORT commands.
2. `DaynaPORT-on-bridge target` - the synthetic network target that runs DaynaPORT commands over the raw bridge.

DaynaPORT targets use the normal global debug setting for bridge tracing, and the logs stay compact so they read like the other SCSI device traces instead of dumping every frame.
